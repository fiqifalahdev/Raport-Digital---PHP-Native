...
if (isset($_POST["submitNilai"])) {
  ...
  $filename = $_FILES["nilaiPeng"]["name"];
  $fileExtension = explode(".", $filename);
  $fileExtension = strtolower(end($fileExtension));
  $fileData = $_FILES["nilaiPeng"]["tmp_name"];
  ...
  $result = mysqli_query($conn, "SELECT * FROM nilai WHERE idkelas = '$classID'");
  ...
  foreach ($reader as $item => $value) {
    ...
    $added = mysqli_query($conn, "INSERT INTO `nilai` (`idmapel`, `idkelas`, `idsiswa`, `harian`, `uas`) VALUES ('$mapelID', '$classID', '$nrp', '$harian', '$uas')");
  }
  $update = addData("UPDATE raport SET status_pengetahuan = 1 WHERE idKelas = $classID");
}
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
  ...
  <!-- CSS Link -->
  <link rel="stylesheet" href="assets/nilaiPengetahuan.css" />
  <!-- Font Link -->
...
<body>
          ...
          <div class="card">
            <div class="card-header d-flex justify-content-between flex-row">
              ...
              <div class="report-name">
                <p class="h4">Nilai Pengetahuan</p>
                <p class="h6"><?= $classname; ?></p>
              </div>
              <div class="navigation d-flex justify-content-evenly">
                <form action="getPengetahuan.php?kelas=<?= $classname; ?>|<?= $classID; ?>" method="post" class="card-nav ms-3">
            ...
            <div class="card-body table-responsive-sm">
              ...
              <?php
              $rows = show("SELECT nilai.idsiswa, siswa.*, nilai.harian, nilai.uas FROM siswa, nilai WHERE siswa.idkelas = $classID AND siswa.idsiswa = nilai.idsiswa AND nilai.idMapel = $mapelID LIMIT $startData, $showedPage");

              $i = $startData + 1;
              ?>
              <?php if (isset($added) && isset($update)) : ?>
        ...
        <div class="modal-content">
          ...
          <form action="" method="post" class="form-input" enctype="multipart/form-data">
            <div class="modal-body">
              <!-- inpput nilai -->
              <input type="file" name="nilaiPeng" id="nilaiPeng" class="form-control">
            </div>
  ...
  <script>
    ...
    const download = document.querySelector("#upload");
